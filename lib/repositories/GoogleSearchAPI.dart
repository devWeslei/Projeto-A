...
class GoogleSearchAPI {
  ...
  List<Item> extractSearchResults(html.Document document) {
    ...
    List<html.Element> h3List = document.querySelectorAll('h3');
    for (html.Element h3 in h3List) {
      String title = h3.nodes[0].nodes[0].text.toString();
      String? href = h3.parentNode?.parentNode?.parentNode?.attributes['href'];
      if (href != null) {
        String? url = href.replaceAll(
            "/url?q=", ""); //corta o inicio do link, começando pelo http...
        url = url.substring(
            0, url.indexOf("&sa")); //corta a string até o valor "&sa"
        result.add(Item(
            title: title,
            link: Uri.decodeFull(Uri.decodeFull(
                url)) // decode 2 vzs porque de alguma forma o google estava fazendo 2 encodes no href.
        ));
      }
    }
    return result;
